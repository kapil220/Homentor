  ...
  return (
    <div
      className={`relative overflow-hidden border-y ${
        isDark
          ? "border-white/10 bg-hommentor-ink text-white"
    ...
    >
      <div
        ...
        className={`pointer-events-none absolute inset-y-0 left-0 w-24 z-10 bg-gradient-to-r ${
          isDark ? "from-hommentor-ink to-transparent" : "from-white to-transparent"
        }`}
      />
      <div
        aria-hidden
          ...
          >
            ...
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                isDark ? "bg-hommentor-gold/70" : "bg-hommentor-blue/40"
              }`}
            />
          </div>
        ))}
